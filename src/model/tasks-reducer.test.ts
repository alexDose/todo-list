...
import {newTodolistId} from './todolist-reducer.test';
...
test('correct task should be deleted', () => {
    const endState = tasksReducer(
        startState,
        deleteTaskAC({ todoId: 'todolistId2', taskId: '2' })
    )
...
test('correct task should be created at correct array', () => {
    ...
    expect(endState.todolistId2[0].id).toBeDefined('todolistId2')
...
test('correct task should change its status', () => {
    const endState = tasksReducer(
        startState,
        changeTaskStatusAC({ todoId: 'todolistId2', taskId: '2', isDone: false })
    )
    expect(endState['todolistId2'][0].isDone).toBe(false)
...
test('correct task should change its title', () => {
    const endState = tasksReducer(
        startState,
        changeTaskTitleAC({ todoId: 'todolistId2', taskId: '2', title: "hello" })
    )
